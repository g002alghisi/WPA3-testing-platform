...
- Test WPA2-Enterprise by means of [`test_e_rogue_wpa2.sh`](Src/test_e_rogue_wpa2.sh). Verify if the device distrusts fake server certificates.
...
- Test WPA3-Enterprise UOSC by means of [`test_e_wpa3.sh`](Src/test_e_wpa3.sh). Set up a WPA3-Enterprise protected AP, thus that announces PMF as required.
...
- Test WPA3-Enterprise TOD by means of [`test_e_rogue_wpa3.sh`](Src/test_e_rogue_wpa3.sh). In this case, the examination is geared towards the Trust Override Disable Policies of the supplicant. In particular, TOD-TOFU policy is enforced by means of the server certificate (`server.pem`).
