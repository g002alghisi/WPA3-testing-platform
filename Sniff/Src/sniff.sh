...
wireshark_setup () {
    ...
    print_info "Setting $wifi_if in monitor mode... "
    net_if_set_monitor_mode -w "$wifi_if" -c "$channel" &&
        print_success || { print_error; return 1; }
}

wireshark_run() {
    print_title "Running Wireshark. Press Ctrl-C to stop."
    sudo wireshark
    print_title "Wiresharkd is stopped."
}

wireshark_setdown() {
    # Set Wi-Fi interface in default mode
    print_info "Setting $wifi_if in default mode... "
    net_if_set_default_mode -w "$wifi_if" &&
        print_success || { print_error; return 1; }

    # Start Network Manager
    print_info "Starting NetworkManager... "
    nm_start && print_success || { print_error; return 1; }
